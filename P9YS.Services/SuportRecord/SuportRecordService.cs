...
namespace P9YS.Services.SuportRecord
{
    public class SuportRecordService : ISuportRecordService
    {
        ...
        public async Task<int> UpdSuportsJob(DateTime endTime)
        {
            //取最后一次标记
            var markTime = await _movieResourceContext.SuportRecords.MaxAsync(s => s.Mark);
            //取时间段内数据
            ...
            var suportRecords = await query.ToListAsync();
            ...
            var suportGroups = suportRecords.GroupBy(s => new { s.MovieQuestionAnswerId })
                ...
                .ToList();
            //查出所有movie
            var movieQuestionAnswers = await _movieResourceContext.MovieQuestionAnswers
