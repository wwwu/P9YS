...
namespace QCloud.CosApi.Api
{
	...
	public class CosCloud
	{
        ...
        public CosCloud(int appId, string secretId, string secretKey, string region, int timeOut = HTTP_TIMEOUT_TIME)
		{
			this.appId = appId;
        ...
        /// sliceSize: 分片大小，可选取值为:64*1024 512*1024，1*1024*1024，2*1024*1024，3*1024*1024
        /// <returns></returns>
        public string UploadFile(string bucketName, string remotePath, byte[] bytes, Dictionary<string, string> parameterDic = null, bool isParallel = false, int minConcurrency = 0)
        {
            if (remotePath.EndsWith("/"))
            {
                return constructResult(ERRORCode.ERROR_CODE_PARAMETER_ERROE, "file path can not end with '/'");
            ...
            //if (fileSize <= SLICE_UPLOAD_FILE_SIZE)
            //{
                return Upload(bucketName, remotePath, bytes, bizAttribute, insertOnly);
            //}
            //else
            //{
            //    //分片上传
		...
		/// <param name="insertOnly">同名文件是否覆盖</param>
		/// <returns></returns>
		public string Upload(string bucketName, string remotePath, byte[] bytes,
                                  string bizAttribute = "", int insertOnly = 1)
        {
            var url = generateURL(bucketName, remotePath);
            var sha1 = SHA1.GetFileSHA1(bytes);
            var data = new Dictionary<string, object>();
            data.Add("op", "upload");
            data.Add("sha", sha1);
            data.Add("biz_attr", bizAttribute);
            ...
            var header = new Dictionary<string, string>();
            header.Add("Authorization", sign);
            return httpRequest.SendRequest(url, ref data, HttpMethod.Post, ref header, timeOut, bytes);
        }
